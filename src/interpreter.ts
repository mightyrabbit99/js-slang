...
import { __expose } from './stdlib/debugger'
...
export type Evaluator<T extends es.Node> = (node: T, context: Context) => IterableIterator<Value>
  ...
  export const evaluators: { [nodeType: string]: Evaluator<es.Node> } = {
    ...
    *CallExpression(node: es.CallExpression, context: Context) {
      if (node.callee.name === "__breakpoint") {
        yield* __expose(node, context)
      }
    ...
    *NewExpression(node: es.NewExpression, context: Context) { const callee = yield* evaluate(node.callee, context)
                                                               const args = []
